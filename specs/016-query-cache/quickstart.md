...
```bash
DB=$(mktemp -d)/vault
./bin/go-rag -c "$DB" config set embedding_model nomic-embed-text   # if not set
./bin/go-rag -c "$DB" add <a-file-or-dir>
# wait for async embeddings to land:
./bin/go-rag -c "$DB" status        # EmbeddingsComplete: true

# Cold query (miss):
time ./bin/go-rag -c "$DB" query "some term" --k 5
./bin/go-rag -c "$DB" status | grep -A3 -i cache    # ResultCache Hits=0/1, Misses=1

# Repeat identical query (hit):
time ./bin/go-rag -c "$DB" query "some term" --k 5
./bin/go-rag -c "$DB" status | grep -A3 -i cache    # Hits incremented; dramatically faster
```
...
```bash
./bin/go-rag -c "$DB" query "unique-new-term" --k 5          # miss (term not in corpus yet)
# add a document containing "unique-new-term":
./bin/go-rag -c "$DB" add <new-file>
./bin/go-rag -c "$DB" status                                  # wait for EmbeddingsComplete
./bin/go-rag -c "$DB" query "unique-new-term" --k 5          # MUST now return the new hit
```
...
```bash
./bin/go-rag -c "$DB" query "some term" --k 5                 # warm the cache
./bin/go-rag -c "$DB" query "some term" --k 5 --no-cache      # bypass serving
```
...
```bash
./bin/go-rag -c "$DB" query "some term" --k 5                 # embeds + caches vector
./bin/go-rag -c "$DB" query "some term" --k 6                 # different k → result miss, but same query text
```
...
```bash
./bin/go-rag -c "$DB" query "some term" --k 5                 # warm both caches
./bin/go-rag -c "$DB" config set embedding_model <other-model>
./bin/go-rag -c "$DB" migrate                                 # re-embeds + flushes both caches
./bin/go-rag -c "$DB" status | grep -A3 -i cache              # both caches Size=0 (flushed)
./bin/go-rag -c "$DB" query "some term" --k 5                 # miss under the new profile
```
