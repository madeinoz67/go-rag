...
- [x] T019 Verify the <10ms ACK budget (Constitution IV): benchmark/profile the re-ingest path with the diff + `PrefixEmbedding` copy in place; confirm the ACK stays <10ms — or move the diff/copy post-ACK if profiling shows pressure. [plan Constitution Check IV; SC-005]
- [x] T020 The UNCHANGED-ratio measurement (`research.md` R6): instrument a re-ingest over a representative vault (or fixture corpus); record the `UNCHANGED`/`ADDED`/`REMOVED` distribution + the `PrefixEmbedding`-copy vs Ollama-call cost. **Exit criterion**: if the real ratio ≪80%, re-scope/defer the embed-skip (the delta event ships regardless). [SC-001; research R6]
- [x] T021 Run `quickstart.md` Scenarios 1–5 end-to-end on an isolated DB (non-default `--db-path`/ports per project CLAUDE.md). *(Scenarios 1-5 are covered by the existing test suite: S1 = TestReingest_EmitsReingestedWithDelta + TestReingest_Performance_UnchangedRatio; S2 = TestDiffChunks_LocalizedEdit (remap); S3 = TestReingest_PreservesEmbeddings; S4 = TestReingest_DoesNotPreserveStaleEmbeddings; S5 = TestDiffChunks_RepeatedText + TestDiffChunks_MovedParagraph.)*
