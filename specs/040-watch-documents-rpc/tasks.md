...
- ⏭ **DEFERRED — `stream.Send` can wedge the handler on a stalled client (high), spec-deferred.** In the `select { case ev := <-ch: stream.Send(…); case <-ctx.Done() }` loop, once `select` commits to the send branch, `ctx.Done()` cannot interrupt a `stream.Send` blocked on HTTP/2 flow control (a connected-but-not-reading client). The handler goroutine + its subscriber then leak until the connection breaks. This is go-rag's first streaming RPC; keepalive tuning + load testing are **Out of Scope** per `plan.md` (the MVP defers them). **Follow-up**: decouple `Send` from the ctx-owned select (sender goroutine + staging channel) and/or enforce grpc server keepalive to bound the wedge. Pre-existing (shipped at `dd9ecf2`), not introduced by the DELETED wiring.

- ⏭ **DEFERRED — no `Bus.Close` → `Engine.Close` can't unblock a live subscriber (high), masked in production.** `Engine.Close` never closes subscriber channels; a live `WatchDocuments` handler blocks on `<-ch` on shutdown UNLESS `grpc.GracefulStop` cancels the stream ctx first (which the daemon does — so the production path is fine). The handler's `!ok` "Bus closed the channel (engine shutdown). Clean exit." branch documents a path that does not exist. **Follow-up**: add `Bus.Close` (write-lock + close every subscriber channel, `sync.Once`) and call it from `Engine.Close` so the documented branch is real and any future in-process (non-gRPC) subscriber unblocks. Pre-existing.
