...
- [X] T018 [US3] Handle multi-page tables (SHIPPED as continuation-marker-split — spec-acceptable "clearly split with continuation markers": a table spanning pages gets "[table continues on next page]" / "[table continued from previous page]" markers. STRICTLY ADDITIVE — detectTables/detectTablesInRun/renderPageWithTables byte-identical (US3 untouched); a new detectTablesStructured sibling feeds a post-ExtractContent annotateContinuations pass. Hard gates: column-count match + X-anchor alignment (colTol) + font parity; any doubt → no marker (false-merge guard tested). The full grid-MERGE is a separate follow-up — it needs a cross-page coordinate space + touches every caller, not worth the regression surface for v1.) in `internal/reader/pdf.go` — extract as one logical table, or split with explicit continuation markers (per spec.md acceptance scenario 3)
...
- **FU-1 — Provider abstraction (ALL 4 packages SHIPPED: embed + enrich + caption + rerank).** Every model-using package has a `New(provider, endpoint, model, apiKey)` factory dispatching Ollama (default) vs OpenAI-compatible (covers OpenAI/Azure/vLLM/LM Studio). Config has per-capability `provider`/`endpoint`/`api_key` fields (default = ollama + OllamaURL = zero change for existing users). File structure: interface+factory in the `*.go` root file; Ollama impl in `ollama.go`; OpenAI impl in `openai.go`. Reranker renamed `Reranker`→`OllamaReranker` for clarity.
...
- **FU-6 — US3 parser fixes (#1 readNumberToken + #7 move-and-show SHIPPED; remaining 6 are safe/non-blocking).** #1: readNumberToken was a greedy scanner consuming +/-/. mid-number (e.g. "1.2.3" → one token → ParseFloat fails → silent 0.0 corrupting positioning); now a strict float scanner (sign + digits + optional fraction + optional exponent — "1.2.3" → 1.2 + 0.3 as two valid tokens). #7: the `'` and `"` move-to-next-line-and-show operators previously dropped their text (hit the default case); now handled as T* + Tj (text emitted, pen advances). The remaining 6 findings (Tz-not-reset-by-BT [spec-correct], identity-scope-creep [documented], TJ one-frag-per-cell [by design], longestUniformRun O(n²) [capped at 60], clusterX single-link [false-negative-safe], octal mod-256 [fragile but correct]) are all safe/non-blocking.
