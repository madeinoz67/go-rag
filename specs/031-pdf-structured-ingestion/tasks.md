...
- **FU-1 — Provider abstraction (captioning SHIPPED; embed/enrich pattern documented).** Captioning now has a provider factory (`caption.New` dispatches Ollama vs OpenAI-compatible) + config (`captioning_provider`/`captioning_endpoint`/`captioning_api_key`). The same pattern extends to embed/enrich (each still has a single Ollama impl — add the factory + OpenAI provider + config fields per capability). See `caption/openai.go` + `caption/captioner.go` `New()`.
