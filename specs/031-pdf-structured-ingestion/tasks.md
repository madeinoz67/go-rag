...
- **FU-1 — Provider abstraction (captioning + embed + enrich SHIPPED; rerank remaining).** All three packages have `New(provider, endpoint, model, apiKey)` factories dispatching Ollama vs OpenAI-compatible (covers OpenAI/Azure/vLLM/LM Studio). Config has per-capability `provider`/`endpoint`/`api_key` fields (default = ollama + OllamaURL = zero change for existing users). File structure refactored: interface+factory in `embedder.go`/`enricher.go`/`captioner.go`; Ollama impl in `ollama.go`; OpenAI impl in `openai.go`. Rerank pending (same pattern; the `rerank.Reranker` needs renaming to `OllamaReranker` + factory + `OpenAIReranker`).
