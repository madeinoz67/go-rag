...
- [ ] T005 [US1] Implement the hard-drift comparison in `internal/engine/drift.go` `computeDriftVerdict(ctx)`: load the baseline (T002); compare baseline.Model vs `cfg.EmbeddingModel`, baseline.Dim vs live `embedderOrOllama().Dimensions()` (skip when live dim 0), baseline.Convention vs `cfg.Prefixer().Convention()`; populate `DriftVerdict` with `Hard=true`, `Verdict="hard-drift"`, and human-readable `Reasons` on any mismatch; `clean` when all match. Wire it into `RefreshDriftVerdict` (replacing the T004 stub).
- [ ] T006 [US1] Wire the boot check in `internal/cli/serve.go`: after `engine.NewWithDB`, call `eng.RefreshDriftVerdict(ctx)` before serving listeners and log the verdict once (`hard-drift: …` / `clean` / etc.) — the loud-at-startup signal (FR-004).
- [ ] T007 [US1] Add readiness to `internal/engine/health.go`: `HealthInfo` gains `Ready bool` (readiness) distinct from `OK bool` (liveness); `Engine.Health` sets `Ready = OK && !cachedVerdict.Hard`.
- [ ] T008 [US1] Expose readiness on the health probes: REST `/health` body in `internal/rest/` includes `ready` + `drift_verdict` (HTTP stays 200 — do NOT 503, per D4); the gRPC `Gorag.Health` RPC maps `Health.Ready` onto a new `ready` field in `proto/gorag.proto` `HealthResponse` (regen `proto/gen`) + `internal/grpc/engine_adapter.go`. **Batch this proto regen with T023** (StatusResponse drift fields) — one generate step.
- [ ] T009 [US1] Test in `internal/engine/drift_test.go`: build a corpus (save a baseline for model A via `SaveBaseline`), reconfigure `cfg.EmbeddingModel` to B, `RefreshDriftVerdict` → assert `Verdict="hard-drift"`, `Hard=true`, `Reasons` mentions model; `Health().Ready==false && Health().OK==true` (the posture-A gate).
- [ ] T010 [US1] Test in `internal/engine/drift_test.go`: a matching baseline (model/dim/convention == live) → `Verdict="clean"`, `Ready==true`.
