...
- **Upstream dependency is met.** MuninnDB issue #556 (UPSERT write mode) merged as PR #659 (`e4d6ad21`, 2026-08-03): `upsert_key` + durable forward index at prefix `0x2F`, cross-surface proto/gRPC/REST/MCP/CLI.
- **The merged ACCUMULATE semantics are safe for this bridge.** Promoted chunks are content-addressed, so the UPSERT key is derived from chunk content. An unchanged chunk re-promoted across restarts/re-ingest lands as `UPDATED_IDENTICAL` — a strict no-op that touches no cognitive state. A changed chunk gets a new identity → a new key → `CREATED`, never `UPDATED_CHANGED`. The bridge never produces the case where ACCUMULATE would forge a learning signal.
