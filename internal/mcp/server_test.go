...
func TestMCP_ToolsListCount(t *testing.T) {
	...
	_ = json.Unmarshal(bytes.TrimSpace(out.Bytes()), &resp)
	tools := resp["result"].(map[string]any)["tools"].([]any)
	if len(tools) != 30 { // spec 035/037/038/039 + spec 045 (5 auth tools) + spec 047 (list_chunks) + spec 050 (delete_document)
	...
	for _, want := range []string{"go_rag_query", "go_rag_status", "go_rag_add", "go_rag_init", "go_rag_scan", "go_rag_config", "go_rag_files", "go_rag_dirs", "go_rag_reprocess", "go_rag_migrate", "go_rag_vault_list", "go_rag_guide", "go_rag_eval", "go_rag_get_chunk", "go_rag_get_chunk_context", "go_rag_batch_get_chunks", "go_rag_list_documents"} {
		if !names[want] {
