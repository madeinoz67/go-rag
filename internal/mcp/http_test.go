...
func TestHTTPToolsList(t *testing.T) {
	...
	if len(env.Result.Tools) != 30 { // spec 045 added 5 auth tools; spec 047 added go_rag_list_chunks; spec 050 added go_rag_delete_document
		t.Fatalf("want 30 tools, got %d", len(env.Result.Tools))
	}
}
