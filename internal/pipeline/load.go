...
func LoadIndex(ws [8]byte, db *storage.DB) (*index.FTS, *index.Vector, error) {
	pebbleDB := db.Pebble()
...
func EmbeddingModelStats(ws [8]byte, db *storage.DB) map[string]int {
	m := map[string]int{}
	lower, upper, err := keys.VaultKindRange(storage.PrefixEmbedding, ws)
	if err == nil {
		_ = db.RangeScan(lower, upper, func(_, val []byte) bool {
