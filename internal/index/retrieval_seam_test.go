...
func TestRetrieval_VectorIndexSeam(t *testing.T) {
	...
	fts.Index(ws, "c2", map[string]string{"body": "alpha note"})
	...
	realR := NewRetrieval(ws, fts, realVec, q)
	fakeR := NewRetrieval(ws, fts, fakeVec, q)
...
func TestNewRetrieval_AcceptsVectorIndex(t *testing.T) {
	...
	r := NewRetrieval(defaultWS(), newTestFTS(t), NewVector(), staticEmbed([]float32{1.0, 0.0}))
	if r == nil {
