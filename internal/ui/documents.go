...
func (s *Server) handleDocumentsList(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.ListDocuments(vaultFromRequest(r), req)
	if err != nil {
...
func (s *Server) handleDocumentDetail(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.GetDocument(vaultFromRequest(r), id)
	if err != nil {
...
func (s *Server) handleDocumentChunks(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.ListChunks(vaultFromRequest(r), id, req)
	if err != nil {
...
func (s *Server) handleChunkContext(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.GetChunkContext(vaultFromRequest(r), chunkID, window)
	if err != nil {
...
func (s *Server) handleDocumentsSearch(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.Query(r.Context(), vault, engine.QueryRequest{Query: q, K: limit})
	if err != nil {
	...
	for _, h := range res.Hits {
		...
		d, err := s.eng.GetDocument(vault, h.DocumentID)
		if err != nil {
			continue
		}
