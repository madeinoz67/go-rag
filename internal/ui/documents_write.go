...
func (s *Server) handleDocumentAdd(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.Add(r.Context(), vaultFromRequest(r), req.Path, req.Glob)
	if err != nil {
...
func (s *Server) handleDocumentRemove(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.eng.DeleteDoc(r.Context(), vaultFromRequest(r), id); err != nil {
...
func (s *Server) handleDocumentReingest(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.eng.GetDocument(vault, id)
	if err != nil {
	...
	sum, err := s.eng.Reprocess(r.Context(), vault, sourcePath)
	if err != nil {
