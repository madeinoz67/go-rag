...
| [BL-005](#bl-005) | Expose section heading in `Chunk` metadata | P1 | S | 1 | ✅ done (spec 041) — `section_depth` (governing heading level 1-6) on `Chunk` + `QueryHit`, all four transports; heading text was already exposed via `section_context` (spec 025); this closes the leaf-level gap (`len(breadcrumb)` ≠ level when headings skip) |
| [BL-006](#bl-006) | Expose extraction quality score in `Chunk` metadata | P1 | S | 1 | ✅ done (spec 042) — `extraction_method` (native\|mixed\|image) + `extraction_quality` (0-1) on `Chunk` + `QueryHit`, all four transports; per-page text-coverage signal; **OCR-limit caveat**: go-rag doesn't OCR, so this is a coverage-confidence signal (a text-bearing scanned PDF reads native; image-only pages yield no chunks) — true OCR-quality detection waits for an OCR story |
