...
| # | Non-Goal | Rationale |
|---|----------|-----------|
| N1 | Cloud/hosted service | Local-only. A separate product could add sync later. **A narrow opt-in loopback egress IS permitted as of spec 060 (MuninnDB bridge): background, off by default, loopback-only (`127.0.0.1:8477` gRPC, refused at config-validation AND at dial — DNS-rebind defense), never a core operation (ingest/index/query never depend on it), async-after-ACK so the `<10ms` write budget is unaffected, circuit-breaker-guarded so a down MuninnDB never wedges the daemon. Promotes content-addressed chunks into a local MuninnDB memory store via UPSERT; the target-vault `mk_` key is read from the `GORAG_BRIDGE_TOKEN` env (never inlined in config, never logged). No remote egress. The same single-operator carve-out pattern as spec 029 (enrichment) / spec 046 (console).** |
...
| N7 | Web UI | CLI only for v1. **A single-operator management console IS in scope as of spec 046** — an embedded, vendored SPA (no Node/Vite/Tailwind build step) served as a fourth loopback transport (`internal/ui`, `--ui-addr` default `127.0.0.1:7881`, empty disables), auth-gated by the spec 045 Bearer-session system (admin login → opaque `gorags_` session, no cookies), for retrieval, bridge-ops dashboards, and all-aspects admin. Hand-written CSS per `docs/style-guide.md` (4-layer theme/base/components/utilities); vendored Alpine.js + Chart.js + Cytoscape distributed via `go:embed`. No TLS — a reverse proxy terminates it. A general-purpose multi-user web app and TUI (bubbletea) remain out of scope. The same single-operator carve-out pattern as spec 029 (enrichment) / spec 032 (bundled embedder) / spec 045 (auth). |
