...
- 2026-07-06 — **Authentication & tokens (spec 045):** single-operator auth shipped — labelled `gorag_` API keys (SHA-256 hashed, secret shown once), an admin user (bcrypt cost 12), opaque `gorags_` Bearer sessions (no cookies — CSRF-free), one `auth.Validate` across REST/gRPC/MCP, a loopback bypass for local "just works" when no credential is minted, and a zero-break legacy `mcp.token` import. **Schema version bumped 2 → 3** (`internal/storage/migrate` registers three new prefixes: `0x17` API-key, `0x18` admin, `0x19` session; `migrate.ExpectedVersion` is now 3). PRD §6.7 gains three rows; §2.2 N2 carves single-operator auth in-scope (multi-user still out). An adversarial 6-lane review (timing/enumeration, bypass-spoof, token-leak, dispatch, cookie/CSRF, concurrency/migration) was run against this surface; findings are tracked in spec 045.
- 2026-08-10 — **MuninnDB bridge (spec 060):** promotes go-rag chunks into a local MuninnDB vault as content-addressed engrams (UPSERT via `idempotent_id`+`upsert_mode`, now that #556/PR #659 merged), auto-backfills the corpus on enable (storm-limited + pausable), surfaces in the console (**retires the last placeholder — all 9 sidebar items live**), degrades safely when MuninnDB is down. Opt-in/loopback-only/never-core-op egress exception — PRD §2.2 N1 carve-out (NOT a constitution amendment; Principle I stands unchanged). Repo's first outbound gRPC client (loopback dialer + Bearer unary+stream interceptor). v1 stateless (no new keyspace prefix, no migration, `ExpectedVersion` unchanged). Transport gRPC; token = env `GORAG_BRIDGE_TOKEN` (never inlined). Built spec-first (26 tasks); `make test -race ./...` all green + lint 0; isolated-daemon smoke verified (MuninnDB down ⇒ `add` ACKs 0.5s, ingest+query functional); code-reviewer pass = approve-with-required-changes (loopback/token-hygiene/async-after-ACK/NFR-002 cognitive-hygiene all RED-verified by the reviewer). Live-MuninnDB E2E + a second adversarial pass pending the operator's running instance.
